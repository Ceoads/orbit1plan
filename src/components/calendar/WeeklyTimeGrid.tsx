    ...
    >
            ...
            return (
              ...
              >
                <span className={cn(
                  ...
                  today ? "font-semibold text-foreground" : "font-medium text-muted-foreground"
                )}>
                  {dayNames[index]}
                </span>
                <span className={cn(
                  "text-sm mt-0.5",
                  today ? "font-bold text-foreground" : "font-medium text-muted-foreground"
                )}>
                  {day.getDate()}
                </span>
                {today && (
                ...
                {hasExam && (
                  <motion.button
                    ...
                    aria-label="Examen"
                  >
                    <div className="w-1.5 h-1.5 rounded-full bg-warning animate-exam-pulse" />
                  </motion.button>
                )}
              </motion.div>
            );
      ...
      <div
        ...
        className={cn(
          ...
          "calendar-scroll-container scrollbar-ios"
        )}
        ...
        <div className="relative">
          {currentHour >= 8 && currentHour <= 20 && (
            <motion.div
              ...
              style={{ top: `${currentTimeOffset * 64}px` }}
              initial={{ opacity: 0, scaleX: 0 }}
              animate={{ opacity: 1, scaleX: 1 }}
              transition={{ delay: 0.5, duration: 0.3 }}
            >
              <div className="flex items-center">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: '#F4845F' }} />
                <div className="flex-1 h-px" style={{ backgroundColor: '#F4845F', opacity: 0.5 }} />
              </div>
            </motion.div>
          )}
          ...
          {timeSlots.map((hour, hourIndex) => (
            ...
            >
              <div
                className="w-10 flex-shrink-0 pr-2 text-right -mt-2 font-medium"
                  ...
                  return (
                    <div key={day.toISOString()} className="relative min-h-full">
                      {dayEvents.map((event) => {
                        ...
                        const name = subject?.name || event.title;
                        ...
                        return (
                          <motion.div
                            key={event.id}
                            className={cn(
                              "absolute inset-x-0 rounded-md overflow-hidden cursor-pointer",
                              "active:scale-95 transition-transform touch-manipulation bg-white"
                            )}
                            style={{
                              height: `${heightMultiplier * 64 - 4}px`,
                              minHeight: '40px',
                              zIndex: 10,
